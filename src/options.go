...
func init() {
	...
	executeRegexp = regexp.MustCompile(
		`(?si)[:+](execute(?:-multi|-silent|-and-exit-on-success)?|reload|preview|change-prompt):.+|[:+](execute(?:-multi|-silent|-and-exit-on-success)?|reload|preview|change-prompt)(\([^)]*\)|\[[^\]]*\]|~[^~]*~|![^!]*!|@[^@]*@|\#[^\#]*\#|\$[^\$]*\$|%[^%]*%|\^[^\^]*\^|&[^&]*&|\*[^\*]*\*|;[^;]*;|/[^/]*/|\|[^\|]*\|)`)
}
